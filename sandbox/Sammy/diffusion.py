...
def sample_prior(batch, sigma, pdb_id, harmonic=True):
    if harmonic:
        bid = batch['ligand'].batch
        ...
        # Retrieve selected_conformer for the given pdb_id
        selected_conformer = None
        if pdb_id in batch.selected_conformers:
            selected_conformer = batch.selected_conformers[pdb_id]

        # Adjust prior dimensions to match selected_conformer
        if selected_conformer is not None:
            if selected_conformer.shape[0] < prior.shape[0]:
                # Pad selected_conformer with zeros to match the size
                padding_size = prior.shape[0] - selected_conformer.shape[0]
                padding = torch.zeros((padding_size, selected_conformer.shape[1]), dtype=selected_conformer.dtype, device=selected_conformer.device)
                selected_conformer = torch.cat([selected_conformer, padding], dim=0)
            elif selected_conformer.shape[0] > prior.shape[0]:
                # Slice selected_conformer to match the size
                selected_conformer = selected_conformer[:prior.shape[0], :]
            prior += selected_conformer
    else:
        # Non-harmonic case
        prior = torch.randn_like(batch["ligand"].pos) * sigma
        if pdb_id in batch.selected_conformers:
            selected_conformer = batch.selected_conformers[pdb_id]
            if selected_conformer.shape[0] < prior.shape[0]:
                # Pad selected_conformer with zeros to match the size
