...
class FlowSiteModule(GeneralModule):
    ...
    def general_step(self, batch, batch_idx):
        ...
        if self.args.flow_matching:
            for pdb_id in batch.pdb_id:
                try:
                    # Check if the selected_conformer attribute exists and is valid
                    if hasattr(batch, 'selected_conformers') and pdb_id in batch.selected_conformers:
                        selected_conformer = batch.selected_conformers[pdb_id]
                        # Ensure dimensionality compatibility
                        if selected_conformer.shape != batch["ligand"].pos.shape:
                            raise ValueError(f"Dimension mismatch: selected_conformer shape {selected_conformer.shape} does not match batch['ligand'].pos shape {batch['ligand'].pos.shape}")
                        x0 = sample_prior(batch, self.args.prior_scale, pdb_id, selected_conformer, harmonic=not self.args.gaussian_prior)
                except Exception as e:
                    # Handle any errors that occurred during processing of selected_conformer
                    print(f"An error occurred with selected_conformer for batch index {batch_idx}: {e}")
                    # Log the error, skip the batch, or take other corrective action as needed
                    return None  # Skip the rest of the processing for this batch
            ...
            try:
                if (self.args.self_condition_inv or self.args.self_condition_x) and np.random.rand() < self.args.self_condition_ratio:
                    ...
                    with torch.no_grad():
                        ...
                        res_pred, pos_list, angles = self.model(copy.deepcopy(batch), x_self=sample_prior(batch, self.args.prior_scale, batch.selected_conformer, harmonic=not self.args.gaussian_prior) if self.args.self_condition_x else None)
    ...
    def flow_match_inference(self, batch, batch_idx=None, production_mode = False):
        ...
        for pdb_id in batch.pdb_id:
            x0 = sample_prior(batch, self.args.prior_scale, pdb_id, harmonic=not self.args.gaussian_prior)
            if self.args.self_condition_inv:
                batch['protein'].input_feat = batch['protein'].feat * 0 + len(atom_features_list['residues_canonical'])
            x_self = sample_prior(batch, self.args.prior_scale, pdb_id, harmonic=not self.args.gaussian_prior) if self.args.self_condition_x else None
            if self.args.self_condition_bit:
                batch.self_condition_bit = torch.zeros((len(batch.pdb_id), 1), dtype=torch.float32, device=self.device)
